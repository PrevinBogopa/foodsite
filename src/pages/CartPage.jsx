...
const Events = () => {
  ...
  const getEventsForDay = (day) => {
    return events.filter(event => new Date(event.date).getDay() === day);
  };
  ...
  }, [date]); // Fetch events whenever `date` changes
            ...
            {daysOfWeek.map((day, index) => {
              ...
              return (
                <div key={index} className="flex flex-col flex-1">
                  <div className={`border-t border-b border-gray-50 py-2 px-24 ${today === dayIndex ? 'bg-orange-100' : ''}`}>
                    <p className="text-center text-gray-500 text-sm font-semibold">{day}</p>
                  </div>
